...
module.exports = {
	...
	init: function (db) {
		...
		this.setupUsers();
		this.setupAccesstokens();
		this.setupPrinters();
		this.setupMaterials();
		this.setupSliceprofiles();
		this.setupModelfiles();
		this.setupPrintjobs();
		this.setupQueue();
	},

	setupModel: function (name, data) {
		this.db.create(name);
		for (i in data) {
			this.db.get(name).create(i, data[i]);
		}
		FormideOS.manager('debug').log('Storage for '+name+' created');
	},
	...
	setupUsers: function () {
		this.setupModel ('users', [
			...
			{
				"username": "norights@printspot.local",
				"password": "password",
				"permissions": "auth"
			}
		]);
	...
	setupAccesstokens: function () {
		this.setupModel ('accesstokens', [
			{
				"token": "b518e7f9-9d7f-4de8-a522-68deaea20024",
				"permissions": "auth,camera,files,led,log,printer,slicer,wifi,device"
			}
		]);
	},

	setupPrinters: function() {
		this.setupModel ('printers', [
			{
				"name": "Mini Builder",
				"buildVolumeX": 200,
				"buildVolumeY": 200,
	...
	setupSliceprofiles: function() {
		this.setupModel ('sliceprofiles', [
			{
				"name": "Default",
				"settings":
				{
	...
	setupModelfiles: function() {
		this.setupModel ('modelfiles', [

		]);
	},

	setupPrintjobs: function() {
		this.setupModel ('printjobs', [

		]);
	},

	setupQueue: function() {
		this.setupModel ('queue', [
